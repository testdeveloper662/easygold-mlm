...
const SendPaymentDeclineEmail = async (req, res) => {
  try {
    // 🚨 CRITICAL LOGGING - API HIT CONFIRMATION
    console.log("=".repeat(80));
    console.log("🚨 DECLINE PAYMENT API HIT: send-payment-decline-email");
    ...
    console.log("🔍 Checking if is_payment_declined column exists...");
    try {
      ...
      if (columnCheck.length === 0) {
        console.log("⚙️ Creating is_payment_declined column...");
        ...
        console.log("✅ is_payment_declined column already exists");
      }
    } catch (columnError) {
    ...
    console.log("📊 Raw query result type:", typeof commissionEntries);
    console.log("📊 Is array?", Array.isArray(commissionEntries));
    ...
    // Check if payment already declined
    const alreadyDeclined = commissionEntries.some((entry) => entry.is_payment_declined === 1);
    if (alreadyDeclined) {
      console.warn("⚠️ Payment already marked as declined for this order and tree");
      return res.status(400).json({
        success: false,
        message: "Payment has already been marked as declined for this order.",
      });
    }

    console.log("✅ Payment not yet declined - proceeding with email sending");
    ...
    for (const broker of nonSellers) {
      ...
      try {
        ...
        // Template variables for email template id=86
        ...
        // Fetch and render email template in RECIPIENT'S language (template id = 86)
        console.log(` Fetching email template id=86 in "${recipientLanguage}" language...`);
        const emailData = await getRenderedEmail(86, recipientLanguage, templateVariables);
    ...
    // Update is_payment_done = 0 and is_payment_declined = 1 for ALL entries in this tree (including seller)
    console.log(
      "💾 Updating database: Setting is_payment_done = 0 and is_payment_declined = 1 for all entries in tree..."
    );
    await db.sequelize.query(
      `
      UPDATE broker_commission_histories
      SET is_payment_done = 0, is_payment_declined = 1
    ...
    console.log("=".repeat(80));
    console.log("🎉 SUCCESS - Payment marked as declined and emails sent!");
    ...
    return res.status(200).json({
      success: true,
      message: "Payment declined and emails sent to all eligible brokers.",
      data: {
        order_id,
        tree,
    ...
    console.error("❌ CRITICAL ERROR in SendPaymentDeclineEmail");
...
module.exports = SendPaymentDeclineEmail;
