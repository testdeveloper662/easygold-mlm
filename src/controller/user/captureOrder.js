    ...
    for (const productType of productTypesToProcess) {
      ...
      if (!isGoldPurchase && !isGoldPurchaseSell && !isGoldFlex && !isEasyGoldToken && !isPrimeInvest && !isGoldPriceFixing && !isDealerPurchasing && !isDealerPurchasingDiamond) {
        for (const pivot of groupPivots) {
          if (isDiamondGemstone) {
            // Diamond/Gemstone: same gross->net VAT-checked calculation as landing_page/my_store,
            // kept as its own self-contained block since diamonds/gemstones live in
            // 6lwup_diamonds/6lwup_gemstones (not 6lwup_product) and use their own shipping table.
            const gemModel = resolveProductType(pivot) === "gemstone" ? db.Gemstones : db.Diamonds;
            const gemProduct = await gemModel.findOne({ where: { id: pivot.product_id } });

            let vatFromProduct = 0;
            if (gemProduct?.VAT && gemProduct.VAT !== "Differenzbesteuert") {
              vatFromProduct = parseFloat(gemProduct.VAT.replace("%", "")) || 0;
            }

            let vatFromCountry = 0;
            const shipping = await db.DiamondOrderShippingOptions.findOne({
            ...
            // ✅ Always use the higher VAT (gold-only rule doesn't apply — diamonds/gemstones have no material column)
